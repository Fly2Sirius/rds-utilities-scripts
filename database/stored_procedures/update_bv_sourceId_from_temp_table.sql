DROP PROCEDURE `datateam`.`backfill_2021116_update_bv_sourceId_from_temp_table`;

DELIMITER ;;
CREATE PROCEDURE `datateam`.`backfill_2021116_update_bv_sourceId_from_temp_table`(inStart int, inIncrement int)
proc_Exit:BEGIN
...
select max(id) into max_temp_id from optimus._temp_borrowerValue_sourceid;
call `datateam`.`job_log_start`("update_bv_sourceId_from_temp_table",job_steps,1,inIncrement,inStart,"Starting Job",@_log_id);
	...
	UPDATE_LOOP:WHILE loop_id <= total_loops DO
		...
		END IF;
	   	SET @sql = CONCAT('update optimus.borrowerValues bv
			join optimus._temp_borrowerValue_sourceid s on bv.id = s.borrowerValueId
			set bv.sourceId = s.sourceId 
			where bv.sourceId = 1
